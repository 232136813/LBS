...
package com.lenovo.vctl.common.lbs.rtree;
...
public class RTree implements SpatialIndex, Serializable {
  ...
  // todo  concurrent 
  //下面两个Map 如果使用 concurrentHashMap 会消除部分多线程错误
  public ConcurrentHashMap<Long, Node> nodeMap = new ConcurrentHashMap<Long, Node>();
  public ConcurrentHashMap<Long,TargetLocation> targetLocationMap = new ConcurrentHashMap<Long,TargetLocation>(5000);
...
//  private TLongStack parents = new TLongArrayStack();
//  private TIntStack parentsEntry = new TIntArrayStack();
  ...
  // which can be reused.
  private TLongStack deletedNodeIds = new TLongArrayStack();
  ...
  public void setLbsManager(LbsManager manager){
	  this.lbsManager = manager;
  }
  //-------------------------------------------------------------------------
   ...
   * </ul></p>
   *
   * @see com.lenovo.vctl.common.lbs.rtree.SpatialIndex#init(Properties)
   */
  public void init(Properties props) {
    if (props == null) {
      ...
      timeSpan = LbsManager.DefaultItemExpireTimeInMill;
    } else {
      ...
      timeSpan = TimeUnit.MINUTES.toMillis(Long.parseLong(props.getProperty("ExpireTimeSpan", LbsManager.DefaultItemExpireTimeInMill+"")));
      if(timeSpan < 0) timeSpan = LbsManager.DefaultItemExpireTimeInMill;
    }
  ...
  public boolean add(TargetLocation loc){
	  if(loc == null)return false;
//		boolean flag = true;
		TargetLocation target = targetLocationMap.get(loc.getTargetId());
		if(target != null){
			if(target.equals(loc))return true;
//			if(!target.miniEquals(loc)){//是不是同一地点的不同时间搜索
//				delete(target.getX(), target.getY(), target.getTargetId());
//			}else{//这里有可能会删除的 同时修改 这里有并发的问题 但是不影响使用
//				if(target.getTimeStamp() < loc.getTimeStamp()){
//					target.setTimeStamp(loc.getTimeStamp());
//				}
//				return true;
//			}
			delete(target.getX(), target.getY(), target.getTargetId());
...
//		if(!flag){
//			log.error("add index error target = " + target + ";loc = " + loc);
//			return false;
//		}
		if(loc.getTimeStamp() <= 0)loc.setTimeStamp(System.currentTimeMillis());
		target = loc;
		targetLocationMap.put(loc.getTargetId(), target);
	    if (log.isDebugEnabled()) {
	      log.debug("Adding rectangle " + loc);
	    }
	    
	    add(target.getX(), target.getY(), target.getX(), target.getY(), target.getTargetId(), 1);
  ...
  public boolean add(float x, float y, long targetId, long timeStamp, byte type){
//		boolean flag = true;
		TargetLocation loc = targetLocationMap.get(targetId);
		if(loc != null){
			 delete(x, y, targetId);
		}
//		if(!flag){
//			log.error("add index error x = " + x+", y = " +y + "; target = " + targetId);
//			return false;
//		}
		if(loc == null){
			loc = new TargetLocation();
		}
		
		loc.setX(x);
		loc.setY(y);
		loc.setTargetId(targetId);
		loc.setType(type);
		loc.setTimeStamp(timeStamp <= 0 ? System.currentTimeMillis() : timeStamp);
		targetLocationMap.put(targetId, loc);
	    if (log.isDebugEnabled()) {
	      log.debug("Adding rectangle " + loc);
	    }

	    add(x, y, x, y, targetId, 1);
  ...
  private void add(float minX, float minY, float maxX, float maxY, long id, int level) {
	...
	TLongStack parents = new TLongArrayStack();
  ...
  /**
   * @see com.lenovo.vctl.common.lbs.rtree.SpatialIndex#delete(Rectangle, int)
   */
  
  
  
  public boolean delete(long targetId){
	  TargetLocation target = targetLocationMap.get(targetId);
	  if(target != null){
		  boolean flag = delete(target.getX(), target.getY(), targetId);
		  if(flag){
			  targetLocationMap.remove(targetId);
			  target = null;
		  }
		  return flag;
	  }
  ...
  public boolean delete(float x, float y, long id){
	  	TLongStack parents = new TLongArrayStack();
	  	TIntStack parentsEntry = new TIntArrayStack();
	  	parents.clear();
	      ...
	      if (!n.isLeaf()) {
	        ...
	        foundIndex = n.findEntry(x, y, x, y, id);
	      }
...
//  /**
//   * @see com.lenovo.vctl.common.lbs.tree.SpatialIndex#nearest(Point, TIntProcedure, float)
//   */
//  public void nearest(float x, float y, TLongProcedure v, float furthestDistance) {
//    Node rootNode = getNode(rootNodeId);
//
//    float furthestDistanceSq = furthestDistance * furthestDistance;
//    nearest(x, y, rootNode, furthestDistanceSq);
//
//    nearestIds.forEach(v);
//    nearestIds.reset();
//  }

  private void createNearestNDistanceQueue(float x, float y, int count, float furthestDistance, final PriorityQueue distanceQueue, long timeSpan) {
    distanceQueue.reset();
...
//    float furthestDistanceSq = furthestDistance;
    ...
    while (parents.size() > 0) {
      ...
      if (!n.isLeaf()) {
        ...
        for (int i = 0; i < n.entryCount; i++) {
          ...
          if (entryDistanceSq <= furthestDistance) {
        	  TargetLocation loc = targetLocationMap.get(entryId);
        	  if(loc == null){//异常
        		  delete(n.entriesMinX[i], n.entriesMinY[i], entryId);//删除这个对象
        		  continue;
        	  }
        	  if((loc.getTimeStamp() + timeSpan) < now){//过期了
        		  //删除过期数据
        		  lbsManager.delayDeleteExpireLocation(entryId);
        		  continue;
        	  }
            ...
            if (distanceQueue.getPriority() < furthestDistance && distanceQueue.size() >= count) {
  ...
  /**
   * @see com.lenovo.vctl.common.lbs.rtree.SpatialIndex#nearestNUnsorted(Point, TIntProcedure, int, float)
   */
  public void nearestNUnsorted(float x, float y, TLongFloatProcedure v, int count, float furthestDistance, final PriorityQueue pq) {
    createNearestNDistanceQueue(x, y, count, furthestDistance, pq, timeSpan);

    while (pq.size() > 0) {
      v.execute(pq.getValue(), pq.getPriority());
      pq.pop();
    }
  }

  /**
   * @see com.lenovo.vctl.common.lbs.rtree.SpatialIndex#nearestN(Point, TIntProcedure, int, float)
   */
  public void nearestN(float x, float y, TLongFloatProcedure v, int count, float furthestDistance,final PriorityQueue pq) {
    createNearestNDistanceQueue(x, y, count, furthestDistance, pq, timeSpan);
    pq.setSortOrder(PriorityQueue.SORT_ORDER_ASCENDING);
  ...
  /**
   * @see com.lenovo.vctl.common.lbs.rtree.SpatialIndex#intersects(Rectangle, TIntProcedure)
   */
  ...
  /**
   * @see com.lenovo.vctl.common.lbs.rtree.SpatialIndex#contains(Rectangle, TIntProcedure)
   */
  ...
  /**
   * @see com.lenovo.vctl.common.lbs.rtree.SpatialIndex#size()
   */
  public int size() {
    return size;
  }
  ...
  /**
   * @see com.lenovo.vctl.common.lbs.rtree.SpatialIndex#getBounds()
   */
  ...
  /**
   * @see com.lenovo.vctl.common.lbs.rtree.SpatialIndex#getVersion()
   */
   ...
   */
//  private float nearest(float x, float y, Node n, float furthestDistanceSq) {
//    for (int i = 0; i < n.entryCount; i++) {
//      float tempDistanceSq = Rectangle.distanceSq(n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i], x, y);
...
//           furthestDistanceSq = nearest(x, y, getNode(n.ids[i]), furthestDistanceSq);
//         }
//      }
//    }
...
//  }
   ...
   */
  private void condenseTree(Node l, TLongStack parents, TIntStack parentsEntry) {
    ...
    TLongStack eliminatedNodeIds = new TLongArrayStack();
   ...
   */
  private Node chooseNode(float minX, float minY, float maxX, float maxY, int level, TLongStack parents, TIntStack parentsEntry) {
   ...
   */
  private Node adjustTree(Node n, Node nn, TLongStack parents, TIntStack parentsEntry) {
  ...
  public TargetLocation getTargetLocation(long id){
	  return targetLocationMap.get(id);
  }
  
  //获取一个快照
  public TLongObjectHashMap<TargetLocation> copyTreeObjects(){
	  TLongObjectHashMap<TargetLocation> copy = new TLongObjectHashMap<TargetLocation>();
	  copy.putAll(this.targetLocationMap);
	  return copy;
  }
